...
describe('Cortex Code Sandbox Runner', () => {
  ...
  it('should block read access to backend/.env file (sandbox constraint)', { skip: !hasSandbox ? 'Sandbox tool missing' : false }, async () => {
...
`;
  ...
  it('should block network access (sandbox constraint)', { skip: !hasSandbox ? 'Sandbox tool missing' : false }, async () => {
    const code = `
