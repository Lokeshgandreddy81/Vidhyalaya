...
export function executeSanitizedUserCode(userCodeString) {
  ...
  const fakeProcess = Object.create(null);
  const fakeEnv = Object.create(null);
  fakeEnv.NODE_ENV = 'production';
  fakeProcess.env = fakeEnv;
  // Erase private master API keys from visibility scope.
  // Do not inject host functions (like exit) to prevent prototype chain traversal.

  executionContextSandbox.process = fakeProcess;
