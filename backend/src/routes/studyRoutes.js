...
const resolveUniversityKey = async (documentId, user) => {
  ...
  if (user && user.role === 'student') {
    if (user.universityId.toLowerCase() !== doc.universityId.toLowerCase()) {
      throw Object.assign(new Error('Unauthorized: Document belongs to a different university'), { status: 403 });
    }
  }
