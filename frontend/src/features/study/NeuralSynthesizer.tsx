...
import {
  ...
  Trophy, ChevronLeft, ChevronRight, Play, Pause, Square, Filter, Star,
  Thermometer, MousePointer, FolderTree, MessageCircle, Search,
  Gamepad2, Volume2, VolumeX
} from 'lucide-react';
import { TransformWrapper, TransformComponent } from 'react-zoom-pan-pinch';
import { chatWithTutor, generateConceptMap, listModels, generateSocraticCheckpoint } from '../../services/geminiService';
...
const VISUAL_MODES: Array<{ id: VisualMode; label: string; icon: React.ReactNode; description: string }> = [
  { id: 'palace', label: 'Mind Palace Arena', icon: <Gamepad2 size={15} />, description: 'Interactive drag-and-drop synapse challenge' },
...
const NODE_COLORS = [
  { fill: '#4e5bff', stroke: '#312e81', text: '#ffffff' },     // Level 0: Foundation (Deep Indigo)
  { fill: '#e0f2fe', stroke: '#0284c7', text: '#0369a1' },     // Level 1: Core Concepts (Cyan Blue)
  { fill: '#f5f3ff', stroke: '#c084fc', text: '#6b21a8' },     // Level 2: Derivatives (Amethyst Lavender)
  { fill: '#f8fafc', stroke: '#94a3b8', text: '#334155' },     // Level 3: Details (Slate Gray)
  { fill: '#f8fafc', stroke: '#cbd5e1', text: '#475569' },     // Level 4+: Nuance (Slate Gray)
];

const ZEN_NODE_COLORS = [
  { fill: '#6366f1', stroke: '#4338ca', text: '#ffffff' },     // Level 0: Foundation
  { fill: 'rgba(14, 165, 233, 0.16)', stroke: 'rgba(14, 165, 233, 0.5)', text: '#e0f2fe' }, // Level 1: Core
  { fill: 'rgba(168, 85, 247, 0.16)', stroke: 'rgba(168, 85, 247, 0.5)', text: '#f3e8ff' }, // Level 2: Derivatives
  { fill: 'rgba(255, 255, 255, 0.06)', stroke: 'rgba(255, 255, 255, 0.22)', text: '#cbd5e1' }, // Level 3: Details
  { fill: 'rgba(255, 255, 255, 0.04)', stroke: 'rgba(255, 255, 255, 0.15)', text: '#94a3b8' }, // Level 4+: Nuance
];
...
function getViewBox(nodes: ConceptNode[], positions: Map<string, Point>, dimensions?: { width: number; height: number }) {
  ...
  if (dimensions && dimensions.width > 0 && dimensions.height > 0) {
    ...
    if (containerRatio > bboxRatio) {
      // Container is wider than bbox: increase width to match ratio
      width = bboxH * containerRatio;
    } else {
      // Container is taller than bbox: increase height to match ratio
...
function getEdgePoint(from: Point, to: Point, metrics: NodeMetrics, extraBuffer: number = 0): Point {
  const dx = to.x - from.x;
  const dy = to.y - from.y;
  if (Math.abs(dx) < 0.1 && Math.abs(dy) < 0.1) return from;

  const width = metrics.width + 12 + extraBuffer * 2; // Add a small buffer for visual clarity
...
const CodeRainCanvas: React.FC<{ isZenMode: boolean }> = ({ isZenMode }) => {
  ...
  if (isSidebar) {
    return (
      <div className={`flex flex-col h-full overflow-hidden transition-colors ${isZenMode ? 'bg-[#0b0f19] text-white' : 'bg-white text-slate-800'}`}>
        ...
        {/* Triple AI Cognitive Lenses Tab Bar - Sidebar Mode */}
  ...
  return (
    ...
    >
      ...
      <div className={`rounded-2xl p-8 border flex-1 overflow-y-auto custom-scrollbar relative flex flex-col gap-6 transition-colors ${
        ...
        {/* Triple AI Cognitive Lenses Tab Bar - Overlay Mode */}
