...
export const generateLearningPlan = async (
  ...
  return apiQueue.add(() => retryWithBackoff(async () => {
    const prompt = `Create a 3-Phase learning roadmap for: "${goal}". Level: "${skillLevel}". Outcome: "${expectedOutcome || 'Mastery'}". Resources: ${resources.substring(0, 2000)}. Return JSON with shape: { title: string, phases: [{ title, description, modules: [{ title, description, estimatedMinutes, keyConcepts: string[] }] }] }`;
...
> **NEXT CONFUSION:** Predict the next mistake the learner will make, then give the one action that prevents it.`;
    ...
    try {
      const response: any = await generateContentWithFallback('text', {
        contents: [{ role: 'user', parts: [{ text: prompt }] }],
      ...
      if (citations.length > 0 && groundingSupports.length > 0 && response?.candidates?.[0]?.content?.parts?.[0]?.text) {
        let originalText = response.candidates[0].content.parts[0].text;
        let injectedText = '';
        let lastIdx = 0;
        
        // Supports are usually sorted by segment start, but let's sort to be safe
        const sortedSupports = [...groundingSupports].sort((a: any, b: any) => 
        ...
        for (const support of sortedSupports) {
          ...
          if (start >= lastIdx && end > start && chunkIndices.length > 0) {
            injectedText += originalText.substring(lastIdx, end);
            
            // Add citation markers
            const validIndices = chunkIndices
              .map((idx: number) => idx + 1)
              .filter((idx: number) => citations.some(c => c.index === idx));
              
            if (validIndices.length > 0) {
              const markers = validIndices.map((idx: number) => `[${idx}]`).join('');
      ...
      console.warn(`[Vidyal.ai] Search-enhanced generation failed, using standard...`, error);
      const response = await generateContentWithFallback('text', {
        contents: prompt
      });
      text = getText(response);
    ...
    if (!text || text.trim().length < 100) {
      throw new Error('Content generation failed — insufficient content returned.');
    }
