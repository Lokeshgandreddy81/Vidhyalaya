...
const ConceptMapRenderer: React.FC<{
...
}> = ({ conceptMap, mode: _mode, onNodeClick, highlightedNode }) => {
  ...
  useLayoutEffect(() => {
    ...
    if (isLinearMode) {
      ...
      const crossGap = mode === 'checklist' ? 96 : mode === 'matrix' ? 150 : nodeCount > 22 ? 140 : 170;
      const layerGap = mode === 'chronos' || mode === 'ladder'
        ? Math.max(360, rootMetrics.width / 2 + 210)
        : mode === 'flow' || mode === 'architect'
          ? Math.max(380, rootMetrics.width / 2 + 230)
          : mode === 'matrix'
            ? Math.max(390, rootMetrics.width / 2 + 240)
            : mode === 'checklist'
              ? Math.max(280, rootMetrics.width / 2 + 155)
              : Math.max(250, rootMetrics.height / 2 + 190);
      const horizontal = ['flow', 'architect', 'chronos', 'ladder', 'matrix', 'checklist'].includes(mode);
      ...
      const placeTree = (id: string, depth: number): number => {
        ...
        if (mode === 'ladder') {
          point = { x: depth * layerGap + depth * 36, y: cross + depth * 70 };
        } else if (mode === 'matrix') {
          const row = Math.floor(cross / crossGap);
          point = { x: depth * layerGap, y: row * crossGap };
        } else if (mode === 'checklist') {
          point = { x: depth * layerGap, y: cross };
        }
  ...
  const getNodeStyle = (node: ConceptNode, isHighlighted: boolean) => {
    const color = NODE_COLORS[Math.min(node.depth, NODE_COLORS.length - 1)];
    const isCentral = node.depth === 0;
  ...
  const renderConnections = () => {
    ...
    return layoutGraph.relationships.map((rel, idx) => {
      ...
      } else if (mode === 'flow' || mode === 'architect' || mode === 'ladder' || mode === 'matrix' || mode === 'checklist') {
  ...
  const renderHUD = (minX: number, minY: number, vW: number, vH: number) => {
    ...
    return (
      <g className="pointer-events-none select-none uppercase font-mono fill-[#000666]" opacity="0.28">
        <text x={minX + 40} y={minY + 52} fontSize="11" fontWeight="700">
        ...
        <g key={node.id} onClick={() => onNodeClick(node)} className="cursor-pointer group">
          ...
          >
            {lines.map((line, index) => (
              <tspan key={line + index} x={pos.x} dy={index === 0 ? 0 : lineHeight}>
                {line}
              </tspan>
            ))}
          </text>
...
const NeuralSynthesizer: React.FC<NeuralSynthesizerProps> = ({
  ...
  useEffect(() => {
    if (moduleTitle && moduleContent && !conceptMap) {
      synthesizeConceptMap();
    }
  }, [moduleTitle, moduleContent, conceptMap]);
                ...
                <>
                  ...
                  <div className="w-full h-full cursor-grab active:cursor-grabbing">
                    <TransformComponent wrapperStyle={{ width: '100%', height: '100%' }}>
                      <div className="w-full h-full min-w-[500px] min-h-[350px]">
                        <ConceptMapRenderer conceptMap={anchoredConceptMap || conceptMap} mode={visualMode} onNodeClick={setSelectedNode} highlightedNode={selectedNode?.id || null} />
                      </div>
                    </TransformComponent>
                  </div>
                </>
