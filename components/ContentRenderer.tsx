...
import {
  ...
  Box, Layers, Sparkles, ChevronRight
} from 'lucide-react';
...
const SHAPE_LIBRARY: Record<GeometryShapeKind, { label: string; accent: string; bg: string; Icon: any }> = {
  ...
  COMPLEXITY_LADDER: { label: 'Complexity Ladder', accent: 'text-violet-700', bg: 'from-violet-50 to-white', Icon: Layers },
...
const MasteryCheckpoint: React.FC<{ moduleTitle?: string; topics?: string[] }> = ({ moduleTitle, topics = [] }) => {
  ...
  return (
    <section
      ...
      className="my-8 border-l-2 border-emerald-400/70 py-1 pl-5 pr-2"
      style={{ breakInside: 'avoid', columnSpan: 'all' } as React.CSSProperties}
    >
      <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="text-[9px] font-black uppercase tracking-[0.24em] text-slate-400">Worth The Space</div>
            <h4 className="mt-1 text-[16px] font-black tracking-tight text-[#000666]">Mastery Checkpoint</h4>
          </div>
        </div>
      ...
      <div className="mt-4 divide-y divide-slate-100">
        {checks.map((check, idx) => (
          <div key={check.label} className="grid gap-2 py-3 sm:grid-cols-[118px_1fr]">
            <div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.18em] text-[#000666]">
              <span className="font-mono text-slate-300">{String(idx + 1).padStart(2, '0')}</span>
              {check.label}
            </div>
            <p className="m-0 text-[14px] leading-relaxed text-slate-600">{check.text}</p>
          </div>
        ))}
      </div>
    </section>
    ...
    <div className="my-8 overflow-hidden rounded-2xl border border-slate-200 bg-[#fbfcff]" style={{ breakInside: 'avoid', columnSpan: 'all' } as React.CSSProperties}>
      <div className="relative min-h-[560px] overflow-hidden">
        ...
        {map.nodes.map(node => (
          <div
            key={node.id}
            className={`absolute -translate-x-1/2 -translate-y-1/2 rounded-full border text-center font-black uppercase shadow-sm ${
              node.root
                ? 'min-w-[360px] border-[#000666] bg-[#000666] px-8 py-5 text-[18px] tracking-[0.04em] text-white shadow-[0_20px_50px_-34px_rgba(0,6,102,0.9)] ring-4 ring-[#000666]/10'
                : node.muted
                  ? 'min-w-[220px] border-slate-200 bg-white/70 px-5 py-3 text-[12px] tracking-[0.02em] text-slate-400'
                  : 'min-w-[240px] border-slate-300 bg-white/95 px-5 py-3 text-[13px] tracking-[0.02em] text-slate-700'
            }`}
...
export const ContentRenderer: React.FC<ContentRendererProps> = ({
  ...
  const cleanContent = (raw: string | null) => {
    if (!raw) return "";
    // Remove AI boilerplate lines like "Architectural Intelligence Report", "Subject:", "Classification:", "System:"
    return removeDuplicateTreeBlocks(raw)
      ...
      .split(/(```[\s\S]*?```)/g)
      .map(part => part.startsWith('```') ? part : stripGeometryScaffold(part))
      .join('')
      .replace(/\n{3,}/g, '\n\n')
      .trim();
  };
  ...
  const MarkdownComponents: any = {
    ...
    h2: ({ children }: any) => {
      ...
      const isActive = matchingSegment?.id === activeSegmentId || (activeLabel && text.toLowerCase().includes(activeLabel.toLowerCase()));
      ...
      const renderChildrenWithCitations = (child: any): any => {
        ...
        if (React.isValidElement(child) && child.props.children) {
          return React.cloneElement(child, {
            children: React.Children.map(child.props.children, renderChildrenWithCitations)
          } as any);
        }
        return child;
      };
  ...
  return (
    <div className="relative w-full h-full bg-white overflow-hidden flex">
      ...
      {focusMode === 'content' && topics.length > 0 && (
        <aside className="group/blueprint relative shrink-0 z-[75] flex w-12 overflow-hidden border-r border-slate-100 bg-[#fcfcfd]/96 transition-[width] duration-500 ease-[cubic-bezier(0.23,1,0.32,1)] hover:w-80">
          <div className="absolute inset-y-0 left-0 flex w-12 flex-col items-center border-r border-slate-100 bg-white/80 pt-32">
            <div className="h-16 w-1 rounded-full bg-gradient-to-b from-teal-400 via-indigo-400 to-transparent opacity-55 transition-all group-hover/blueprint:opacity-100" />
            <div className="mt-5 select-none text-[8px] font-black uppercase tracking-[0.26em] text-slate-300 transition-colors group-hover/blueprint:text-[#000666] [writing-mode:vertical-rl]">
              Blueprint
            </div>
          </div>
      ...
      >
        ...
        {focusMode === 'content' && (
          <>
            <div className="absolute top-0 left-0 w-64 h-64 bg-indigo-50/30 blur-[100px] pointer-events-none" />
            <div className="absolute bottom-0 right-0 w-96 h-96 bg-slate-50/50 blur-[120px] pointer-events-none" />
          </>
        )}

        <div 
          className={`relative transition-all duration-1000 ease-[cubic-bezier(0.16,1,0.3,1)] will-change-[width,opacity] ${isTransitioning ? 'opacity-0 scale-[0.992] blur-[1px]' : 'opacity-100 scale-100 blur-0'} ${focusMode === 'content' ? 'mr-auto ml-0 max-w-[1200px] px-4' : 'mx-auto max-w-[850px]'}`}
          style={{ scrollbarGutter: 'stable', contain: 'layout' } as any}
        >
          {isLoading ? (
            <div className="animate-pulse space-y-8">
              <div className="h-12 bg-slate-100 rounded-lg w-2/3" />
            ...
            <div className={`relative ${focusMode === 'content' ? 'book-spread-mode' : ''}`}>
              <div className={`prose-slate prose-lg max-w-none transition-all duration-800 ease-[cubic-bezier(0.23,1,0.32,1)] ${showColumns ? 'lg:columns-2 lg:gap-32' : ''}`}>
                <ReactMarkdown 
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {citations.map((c, i) => (
                      <a 
                        ...
                        className="group flex flex-col p-5 rounded-2xl border border-slate-200/60 bg-slate-50/50 hover:bg-white hover:border-indigo-200 hover:shadow-lg transition-all text-left"
                      >
