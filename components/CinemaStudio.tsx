...
const CinemaStudio: React.FC<CinemaStudioProps> = ({
  videoId,
  allVideoIds = [],
  moduleTitle,
  moduleContent,
                ...
                <div className="relative isolate aspect-video w-full bg-black">
                  {!allFailed ? (
                    ...
                    <div className="absolute inset-0 flex flex-col items-center justify-center p-8 text-center">
                      <AlertTriangle size={34} className="mb-4 text-amber-400" />
                      <h3 className="text-lg font-black text-white">This video source is restricted.</h3>
                      <p className="mt-2 max-w-md text-sm font-medium text-white/55">Try resyncing the lesson to find another embeddable learning source.</p>
                      <button
                        onClick={handleReSync}
                        disabled={isSyncing}
                        className="mt-5 rounded-full bg-white px-5 py-2.5 text-[11px] font-black uppercase tracking-[0.16em] text-slate-950 disabled:opacity-50"
                      >
                        {isSyncing ? 'Resyncing...' : 'Re-scout source'}
                      </button>
                    </div>
                  )}
                  {!allFailed && (
              ...
              >
                ...
                {!allFailed ? (
                  ...
                  <div className="absolute inset-0 flex flex-col items-center justify-center bg-[#0a0a0f] p-8 text-center">
                    ...
                    <h3 className="text-white text-[14px] font-black uppercase tracking-[0.2em] mb-3">Feed Restricted</h3>
                    <button onClick={handleReSync} disabled={isSyncing}
                      className="px-8 py-3 bg-white/10 text-white border border-white/20 rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-white/20 transition-all">
                      Re-Scout Source
                    </button>
                  </div>
                )}
