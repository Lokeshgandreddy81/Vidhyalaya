...
### Bug 2: Autoscroll Hijacks Manual Log Inspection

- **Symptom**: If the user scrolls up in the log history to review previous command outputs, any new output (e.g. server console ticks, compile steps, metrics changes) instantly scrolls the viewport back to the bottom.
- **Root Cause**: `useEffect` listening to `activeSession.history` forces `scrollTop = scrollHeight` unconditionally on every single change.
- **Steps to Reproduce**:
  1. Run `help` to generate a page of logs.
  2. Scroll up to the top.
  3. Wait for the 1.5-second metrics tick or run another background step. The terminal immediately scrolls to the bottom.
- **Resolution Strategy**:
  - Capture scroll position and height before scroll state updates.
  - Implement a scroll-bottom detection threshold: `isAtBottom = (scrollHeight - scrollTop - clientHeight) < 15`.
  - Only execute autoscroll if `isAtBottom` was true prior to the state update, preserving user-scrolled views.

---

### Bug 3: Lack of ANSI Escape Code Decoder

- **Symptom**: Raw output from compiler scripts, node tests, or linter errors containing standard ANSI colors (`\x1b[31m` for red, `\x1b[32m` for green, etc.) render as plain text strings showing raw garbage characters.
- **Root Cause**: No ANSI code parsing is done. The terminal renders plain strings with primitive prefix tests (`log.startsWith('ERROR:')`).
- **Steps to Reproduce**:
  - Run any build output containing ansi codes. Raw escape characters are displayed.
- **Resolution Strategy**:
